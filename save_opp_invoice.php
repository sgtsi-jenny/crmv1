	...
	if(!empty($_POST)){
		...
		if($errors!=""){
			...
			if(empty($inputs['id'])){
				...
				die;
				redirect("frm_purchases.php?id=".urlencode($inputs['id']));
			}
			die;
		}
		else{
			...
			if(empty($inputs['opp_quote'])){
				...
				redirect("opp_invoice.php"."?id={$inputs['opp_id']}");
				...
				$con->myQuery("UPDATE files SET title=:title,description=:description,user_id=:user,opp_id=:opp_id, date_modified=:date_modified,document='$name' WHERE id=:opp_quote",$inputs);
