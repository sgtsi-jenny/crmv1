<?php
    ...
    if(empty($_GET['id'])){
        ...
        $account=$con->myQuery("SELECT id,account_name,industry,address,account_handler,account_status,phone,date_created,date_modified,email_address FROM qry_accounts WHERE id=?",array($_GET['id']))->fetch(PDO::FETCH_ASSOC);
        if(empty($account)){
